...
export default function CustDrillModal({ custName }: { custName: string }) {
  ...
  return (
    <>
      ...
      <div className={styles.tw}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
        <thead><tr style={{background:'var(--surface)'}}>{['Invoice','Date','Revenue','Margin %','Items'].map(h=><th key={h} style={{padding:'8px 12px',textAlign:'left',fontSize:10,fontWeight:700,letterSpacing:'.08em',textTransform:'uppercase',color:'var(--text-muted)',borderBottom:'1px solid var(--border)'}}>{h}</th>)}</tr></thead>
        <tbody>{invs.map((inv:any)=>{ const im=inv.rev?inv.marg/inv.rev*100:0; return <tr key={inv.bill}><td className="mono" style={{padding:'8px 12px',borderBottom:'1px solid var(--surface-2)'}}>{inv.bill}</td><td className="mono" style={{padding:'8px 12px',borderBottom:'1px solid var(--surface-2)',color:'var(--text-secondary)'}}>{inv.date||'—'}</td><td className="mono" style={{padding:'8px 12px',borderBottom:'1px solid var(--surface-2)',color:'var(--text-secondary)'}}>{omr(inv.rev)}</td><td style={{padding:'8px 12px',borderBottom:'1px solid var(--surface-2)'}}><span className={`badge badge-${im>=50?'green':im>=25?'amber':'red'}`}>{pct(im)}</span></td><td className="mono" style={{padding:'8px 12px',borderBottom:'1px solid var(--surface-2)',color:'var(--text-secondary)'}}>{inv.items}</td></tr>})}
        </tbody>
      </table></div>
    </>
  )
