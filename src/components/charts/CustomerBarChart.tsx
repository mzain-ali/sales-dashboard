...
interface DataItem { name: string; value: number; fullName?: string }
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} layout="vertical" margin={{ left: 8, right: 8 }}>
        ...
        <Bar dataKey="value" radius={[0,4,4,0]} barSize={16} onClick={(d: any) => {
          if (d) {
            onBarClick?.(d.fullName || d.payload?.fullName || d.name || d.payload?.name)
          }
        }} cursor={onBarClick ? 'pointer' : undefined}>
          {data.map((entry, i) => {
            const dimmed = activeFilter && activeFilter !== (entry.fullName || entry.name)
